...
const seedUsers = async () => {
  try {
    const uri = process.env.MONGODB_URI || process.env.MONGODB_URI_ALT;
    ...
    // Create Officer
    const officerExists = await User.findOne({ email: 'officer@vendorbridge.com' });
    if (!officerExists) {
      await User.create({
    ...
    let vendorProfile = await Vendor.findOne({ email: 'vendor1@vendorbridge.com' });
    if (!vendorProfile) {
    ...
    const vendorExists = await User.findOne({ email: 'vendor1@vendorbridge.com' });
    if (!vendorExists) {
      await User.create({
      ...
      if (!vendorExists.vendorId) {
        vendorExists.vendorId = vendorProfile._id;
      }
